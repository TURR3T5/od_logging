...
import { Container, Title, Text, Paper, Group, Box, Button, Tooltip, SimpleGrid, Card, Badge, Switch, Checkbox, Divider, ActionIcon, ColorSwatch, LoadingOverlay } from '@mantine/core';
import { MapPin, Buildings, Car, ShieldStar, Users, Storefront, CirclesThree, Tree, Question, MagnifyingGlass, Funnel, SquaresFour } from '@phosphor-icons/react';
...
// These values will need to be adjusted based on your actual map image and game coordinates
const MAP_BOUNDS = {
...
const createPopupContent = (location: MapLocation) => {
	return `
		<div style="padding: 5px; max-width: 200px;">
			...
			<p style="margin: 0 0 5px 0; font-size: 12px;">${location.type.charAt(0).toUpperCase() + location.type.slice(1)}</p>
			${location.description ? `<p style="margin: 0 0 8px 0; font-size: 12px;">${location.description}</p>` : ''}
			${location.owner ? `<p style="margin: 0 0 5px 0; font-size: 12px;"><strong>Ejer:</strong> ${location.owner}</p>` : ''}
			${location.isForSale && location.price ? `<p style="margin: 0; font-size: 12px;"><strong>Pris:</strong> $${location.price.toLocaleString()}</p>` : ''}
			<p style="margin: 5px 0 0 0; font-size: 11px;">Koordinater: ${location.position.x.toFixed(0)}, ${location.position.y.toFixed(0)}</p>
		</div>
	`;
};
...
export default function CityMapPage() {
	...
	// Mock data for development - replace with Supabase query in production
	...
	}, [locations, visibleTypes, showOnlyForSale, showOnlyPublic, searchQuery]);
	...
	useEffect(() => {
		...
		if (mapContainer.current) {
			...
			map.current = new maplibregl.Map({
				...
				style: {
					...
					sources: {
						'gta-map': {
							type: 'raster',
							tiles: [
								// Replace with your actual GTA V map tiles URL
								tileServerUrl,
							],
							tileSize: 256,
				...
				dragRotate: false, // Disable 3D rotation (keeping 2D view)
	...
	const addMarkerToMap = (location: MapLocation) => {
		...
		popups.current[location.id] = popup;
		...
		element.addEventListener('mouseleave', () => {
			// Remove popup with slight delay to allow moving to popup content
			setTimeout(() => {
	...
	// Only show markers in the current viewport
	...
	const setWaypoint = (location: MapLocation) => {
		...
		// In production, this would call your API to set a waypoint in FiveM
		// Example:
		/*
		fetch('/api/set-waypoint', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				'X-API-KEY': process.env.FIVEM_API_KEY
			},
			body: JSON.stringify({
		...
		<MainLayout requireAuth={false}>
			<Container size='xl' py='xl'>
				...
				<Group justify='space-between' mb='md'>
					...
					<Group>
						<Tooltip label='Skift visning'>
							<ActionIcon variant='subtle' onClick={() => setViewMode(viewMode === 'map' ? 'grid' : 'map')}>
								{viewMode === 'map' ? <SquaresFour size={18} /> : <MapPin size={18} />}
							</ActionIcon>
						</Tooltip>
							...
							<Box>
								...
								{viewMode === 'map' && (
									<>
										<Divider my='md' />
										<Group justify='space-between'>
											<Text size='sm'>Clustering</Text>
											<Switch checked={clusteringEnabled} onChange={(event) => setClusteringEnabled(event.currentTarget.checked)} />
										</Group>
									</>
								)}
							</Box>
								...
								{filteredLocations.map((location) => (
									<Card key={location.id} shadow='sm' padding='lg' radius='md' withBorder>
										<Group justify='space-between' mb='xs'>
											...
											<Badge color={location.type === 'property' ? 'red' : location.type === 'business' ? 'blue' : 'green'}>{typeLabels[location.type]}</Badge>
										</Group>
										...
										{location.owner && (
											<Text size='sm' mb='xs'>
												<strong>Ejer:</strong> {location.owner}
											</Text>
										)}
										...
										{location.isForSale && location.price && (
											<Text size='sm' mb='md'>
												<strong>Pris:</strong> ${location.price.toLocaleString()}
											</Text>
										)}

										<Group gap='xs' mb='xs'>
											<Text size='xs' c='dimmed'>
												X: {location.position.x.toFixed(0)}, Y: {location.position.y.toFixed(0)}
											</Text>
										</Group>

										<Button variant='light' fullWidth mt='md' onClick={() => setWaypoint(location)} leftSection={<MapPin size={16} />}>
											Sæt Waypoint
										</Button>
									</Card>
								))}
